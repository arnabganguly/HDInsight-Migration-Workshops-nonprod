...
object ClientArgumentKeys extends Enumeration {
  ...
  val NumConsumers: String = "consumers"
}
...
object ClientArgumentParser {
  ...
  def usageExample(): Unit = {
    ...
    println(s"Usage: --bootstrap-servers $bootstrapServers" +
      s" --topic $topicName --group-id $groupID, --consumers $numConsumers")
    println()
  }
  ...
  def parseArguments(argumentMap: ArgumentMap, argumentList: List[String]): ArgumentMap = {
    ...
    argumentList match {
      ...
      case "--group-id" :: value :: tail =>
        parseArguments(argumentMap ++ Map(Symbol(ClientArgumentKeys.GroupID) -> value.toString), tail)
      case "--consumers" :: value :: tail =>
        parseArguments(argumentMap ++ Map(Symbol(ClientArgumentKeys.NumConsumers) -> value.toInt), tail)
      case option :: tail =>
        println()
  ...
  def verifyArguments(argumentMap: ArgumentMap): Unit = {
    ...
    assert(argumentMap.contains(Symbol(ClientArgumentKeys.GroupID)))
    assert(argumentMap.contains(Symbol(ClientArgumentKeys.NumConsumers)))
  }
}
